...
const messages: Record<string, string> = {
  limit:
    "Aday kümesi güvenli işlem sınırını aşıyor; sonuçlar daraltılmadan hesaplanamaz.",
...
export default async function CustomerRequestDetail({
  ...
  searchParams: Promise<{ matching?: string }>;
}) {
  const { id } = await params;
  const feedback = (await searchParams).matching;
  const model = await new PostgresMatchingReadRepository().get(
  ...
  return (
    <section className="space-y-6">
      <header>
        <h1 className="text-2xl font-semibold">Müşteri talebi</h1>
        <p className="text-muted-foreground text-sm">
          Deterministik ilan eşleşmeleri
        </p>
      </header>
      <section
        className="rounded border p-4"
        aria-labelledby="matching-profile"
      >
        <h2 id="matching-profile" className="font-semibold">
          Matching V2 profili
        </h2>
        <dl className="mt-3 grid gap-3 sm:grid-cols-2">
          {Object.entries(labels).map(([key, label]) => (
            <div key={key}>
              ...
              <dd>
                {matchingCriterionLabel(
                  model.profile[key as keyof typeof model.profile],
                )}
              </dd>
            </div>
          ))}
      ...
      <section className="space-y-3" aria-labelledby="matching-results">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            <h2 id="matching-results" className="font-semibold">
              Eşleşmeler
            </h2>
            <p role="status" className="text-muted-foreground text-sm">
              {hasStale
                ? "Eski sonuçlar mevcut; yeniden hesaplama önerilir."
                : hasCurrent
                  ? "Sonuçlar güncel."
                  : "Henüz hesaplama yapılmadı."}
            </p>
          </div>
          <form action={calculateMatchesAction}>
            <input type="hidden" name="customerRequestId" value={model.id} />
            <button className="rounded border px-3 py-2">
              {hasCurrent ? "Yeniden hesapla" : "Eşleşmeleri hesapla"}
            </button>
          </form>
        </div>
        {model.results.length === 0 ? (
          <p className="text-muted-foreground rounded border p-4">
            Henüz kaydedilmiş eşleşme yok.
          </p>
        ) : (
          <ol className="space-y-3">
            {model.results.map((result, index) => (
              <li key={result.propertyId} className="rounded border p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <Link
                      ...
                      href={`/admin/properties/${result.propertyId}`}
                    >
                ...
                <ul className="mt-3 list-disc pl-5 text-sm">
                  {result.reasonCodes.map((code) => (
                    <li key={code}>{matchingReasonLabel(code)}</li>
                  ))}
                </ul>
              </li>
            ))}
