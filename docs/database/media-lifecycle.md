...
| `property_media_variants` | Immutable versioned variant metadata keyed to media plus recipe/version/width/format, dimensions, bytes, checksum, and R2 key. A uniqueness rule prevents two rows for the same media/version/recipe/width/format. |
...
| `UPLOADED -> DELETED` | Authorized actor cancels before processing or security/privacy control quarantines it | Lock/version check; deletion metadata/audit; durable private-object purge after retention. Never public. |
...
| `PROCESSING -> DELETED` | Authorized takedown/cancellation wins over active work | Lock/version check; invalidate the claim/version so late completion cannot win; set deletion metadata/audit; durable revocation/purge intent. |
...
| `READY -> DELETED` | Authorized removal, privacy takedown, or parent lifecycle action removes media | Lock/version check; set deletion metadata/audit; durable immediate delivery/cache revocation plus later idempotent object purge. |
...
| `FAILED -> DELETED` | Authorized actor abandons failed media | Lock/version check; deletion metadata/audit; durable retained-object cleanup. |
| `DELETED -> UPLOADED` | Authorized actor restores within retention for full revalidation | Original exists and checksum matches; not erased, held against restore, or replaced incompatibly; property exists and actor remains authorized | Lock/version check; clear only approved deletion fields, reset visibility/cover/order eligibility, return to `UPLOADED`, audit, and enqueue processing only after commit. Never restore directly to `READY`. |
...
- **Open Decision:** Responsive recipes, compatibility fallback, crop policy, byte budgets, and seamless reprocessing/version promotion.
...
- **Open Decision:** R2 topology, delivery/revocation mechanism and hard-removal SLO, retention, restore window, legal hold, and privacy-erasure periods.
