...
- **CRM and appointments:** V1 has no identity-to-customer link or customer-account policy. Advisor scope derives from `customers.assigned_advisor_id`; dependent contacts, requests, activities, appointments, and matches inherit scope only through indexed FKs to that customer/request. Leads use their explicit assigned advisor. No submitted advisor/customer ID grants scope. A future V2 customer account or per-request/shared-team assignment model requires a schema and matrix change. PII stays unavailable to anonymous actors and unassigned advisors. Merge, export, erase, assignment, and restore remain application commands even if a row predicate could be expressed.
