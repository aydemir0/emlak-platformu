...
- In V1, business-record soft-delete and restore policies permit only `ADMIN`; advisor end, detach, cancel, dismiss, and deactivate commands are distinct lifecycle operations and do not grant delete/restore authority.
...
| Authenticated customer | Customer/public accounts do not exist in V1. This reserved V2 principal receives no customer-only policy in V1; public access remains anonymous and server-mediated.                                                                                                                       |
| Advisor (`ADVISOR`)    | Current explicit permissions plus current object scope are required. Advisor identity alone grants neither global CRM visibility nor export/delete/restore/audit authority. Publish/unpublish requires the explicit target-property permission. Advisor MFA is optional at launch.         |
| Admin (`ADMIN`)        | Current administrative permission is still checked by action and object. Business-record soft delete/restore is `ADMIN`-only. Admin access remains field-, lifecycle-, and audit-constrained, and MFA is mandatory before production.                                                      |
...
- **CRM and appointments:** V1 has no identity-to-customer link or customer-account policy. Advisor scope derives from `customers.assigned_advisor_id`; dependent contacts, requests, activities, appointments, and matches inherit scope only through indexed FKs to that customer/request. Leads use their explicit assigned advisor. Phase 8 lead intake, activity/history writes, and staff commands are server-mediated; no anon CRM base-table grant or policy exists. Advisors can read only assigned lead/activity/history rows and application authorization separately denies assignment/reassignment. No submitted advisor/customer ID grants scope. A future V2 customer account or per-request/shared-team assignment model requires a schema and matrix change. PII stays unavailable to anonymous actors and unassigned advisors. Merge, export, erase, assignment, and restore remain application commands even if a row predicate could be expressed.
...
- Staff role codes are exactly `ADMIN` and `ADVISOR`; their precise permission bundles and object scopes remain to be approved.
- Soft-delete visibility and purge follow [retention and deletion](retention-deletion.md).

## Open Decisions

- Exact `ADMIN`/`ADVISOR` permission bundles, object scopes, approval/dual-control rules, and whether any advisor access derives from assignment versus a broader operational permission.
- V2 customer-account identity linking and recovery design; no customer account or customer-link policy ships in V1.
...
- Which high-impact commands require recent authentication or an approval step beyond the baseline, and whether advisor MFA later becomes mandatory. Admin MFA is required before production.
