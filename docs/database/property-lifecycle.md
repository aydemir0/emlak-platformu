...
The Properties domain owns property identity, commercial/publication state, canonical slug, price history, and advisor association. PostgreSQL is authoritative. This document refines [domain boundaries](../architecture/domain-boundaries.md), [SEO architecture](../architecture/seo-architecture.md), [caching strategy](../architecture/caching-strategy.md), and [ADR-002](../decisions/ADR-002-supabase-postgresql.md). V1 serves one organization, has no multi-tenancy, and adds no tenant key, per [ADR-009](../decisions/ADR-009-future-multi-tenancy-boundary.md).
...
| `property_slug_history` | Immutable prior normalized slugs, validity/redirect timestamps, and `route_reservation_id` referencing the SEO/content-owned `public_route_reservations`. One current canonical slug is held by `properties`; every old property slug/route resolves directly to the current canonical route with `301`, without redirect chains. Old routes remain globally reserved for redirects. |
...
Publication and unpublication commands may be performed only by an `ADMIN` or an `ADVISOR` holding the explicit permission for the target property. Property soft delete and restore are `ADMIN`-only commands.
...
| `ACTIVE -> REVIEW` | `ADMIN` or explicitly authorized `ADVISOR` unpublishes for material correction/reapproval | No unresolved reservation; reason supplied | Lock/version-check; append audit; durable revocation/invalidation for page, lists, sitemap, and media eligibility. |
...
- The canonical V1 property route is `/satilik|kiralik/{city}/{district}/{property-type}/{slug}`. Slug changes lock the property and relevant `public_route_reservations` rows in deterministic normalized-route order, then verify expected property version and global route ownership. In one transaction they acquire the new SEO/content-owned reservation, insert the old slug plus its retained `route_reservation_id` into `property_slug_history`, update the canonical slug/version, and append audit/outbox records. The old reservation is not released for unrelated reuse; historical lookups return `301` directly to the current canonical route.
...
- **Open Decision:** Exact publication completeness, whether selected publication actions need dual control, reservation evidence, and advisor assignment cardinality. Publish/unpublish actor eligibility is locked to `ADMIN` or an explicitly authorized `ADVISOR`.
...
- **Open Decision:** Slug normalization details, route-reservation ownership transfer/correction operations, exact legal retention/legal-hold periods, and privacy-erasure details. Historical public routes remain reserved and return `301` to the current canonical property route; any exceptional reuse requires a separate SEO decision and redirect-safety analysis.
